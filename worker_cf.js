// ============================================================================
// Cloudflare Worker «Школьный Хаб»: /moderate + /parse/google + /parse/nikasoft
// ----------------------------------------------------------------------------
...
//
// Без npm-зависимостей: только Web API (fetch, DecompressionStream) и
// собственный лёгкий XML/HTML-парсер вместо DOMParser (в Workers его нет).
//
// Переменные окружения (Workers -> Settings -> Variables / secrets):
...
const FIREBASE_API_KEY = (env) => (env && env.FIREBASE_API_KEY) || "";
const FIREBASE_DB = (env) =>
  ((env && env.FIREBASE_DB_URL) || DEFAULT_DB).replace(/\/+$/, "") + "/";

// idToken анонимной сессии (как в parse_schedule.py: accounts:signUp).
async function firebaseToken(env) {
  const key = FIREBASE_API_KEY(env);
...
async function fbGet(env, path) {
  const res = await fetch(FIREBASE_DB(env) + path + ".json");
  if (!res.ok) return null;
  return await res.json();
}

async function fbExists(env, path) {
  return fbGet(env, path).then((v) => v != null);
}

async function fbPut(env, path, obj) {
  console.log("Записываю в Firebase… (" + path + ")");
  const token = await firebaseToken(env);
  const url = FIREBASE_DB(env) + path + ".json?auth=" + encodeURIComponent(token);
  const res = await fetch(url, {
    method: "PUT",
...
// МСК = UTC+3 (в Workers Time.now() — UTC, сдвигаем вручную).
...
async function handleModerate(body, env) {
  const key = env && env.XKIRO_API_KEY;
...
// ============================================================================
// Лёгкий XML/HTML-парсер (замена DOMParser, недоступного в Cloudflare Workers)
// ============================================================================
...
function decodeXmlEnts(s) {
  ...
  return s.replace(/&(#\d+|#x[0-9a-fA-F]+|[a-zA-Z]+);/g, (m, e) => {
    try {
      ...
      return XML_ENT[e] != null ? XML_ENT[e] : m;
    } catch {
      return m;
    }
  });
...
// Ленивый парсер разметки: терпимый к незакрытым/вложенным тегам (как у HTML).
function parseMarkup(src) {
  const root = new XNode("#root", {});
  const stack = [root];
  let i = 0;
  const n = src.length;
  ...
  while (i < n) {
    if (src[i] !== "<") {
      ...
      if (j < 0) { appendText(src.slice(i)); break; }
      appendText(src.slice(i, j));
      i = j;
      continue;
    }
    if (src.startsWith("<!--", i)) {
    ...
    if (src.startsWith("<![CDATA[", i)) {
      ...
      if (j < 0) { appendText(src.slice(i + 9)); break; }
      appendText(src.slice(i + 9, j));
      i = j + 3;
      continue;
    }
    if (src.startsWith("<?", i)) { // XML declaration / PI
      const j = src.indexOf("?>", i + 2);
      i = j < 0 ? n : j + 2;
      continue;
    }
    if (src.startsWith("<!", i)) { // DOCTYPE и прочее
      const j = src.indexOf(">", i);
      i = j < 0 ? n : j + 1;
      continue;
    }
    if (src.startsWith("</", i)) { // закрывающий тег
      const gt = src.indexOf(">", i);
      const name = (gt < 0 ? src.slice(i + 2) : src.slice(i + 2, gt)).trim().split(/\s/)[0];
    ...
    // открывающий тег: ищем '>' за пределами кавычек
    let gt = i + 1;
    let inQ = null;
    while (gt < n) {
      const ch = src[gt];
    ...
    const mm = /^([^\s\/=]+)([\s\S]*)$/.exec(body);
    const name = mm ? mm[1] : "";
    const attrs = {};
    if (mm) {
      const ar = /([^\s=\/>]+)(?:\s*=\s*(?:"([^"]*)"|'([^']*)'))?/g;
      let m2;
      while ((m2 = ar.exec(mm[2])) !== null) {
        if (m2[1] === "/" || m2[1] === "") continue;
        const val = m2[2] != null ? m2[2] : (m2[3] != null ? m2[3] : "");
        attrs[m2[1]] = decodeXmlEnts(val);
      }
    }
    const node = new XNode(name, attrs);
...
const DEFAULT_GOOGLE_SHEETS_ID = "1qJ6eBCLSWkbQSwqtD2j-70kRf3F8d0IUU-MdP3MQBxQ";
...
async function loadXlsxSheets(bytes) {
  ...
  const wb = parseMarkup(new TextDecoder().decode(workbookXml));
  const rels = parseMarkup(new TextDecoder().decode(relsXml || ""));
  ...
  if (sstrXml) {
    const sd = parseMarkup(new TextDecoder().decode(sstrXml));
...
async function handleParseGoogle(body, env) {
  const errors = [];
  ...
  const sheetId = (env && env.GOOGLE_SHEETS_ID) || DEFAULT_GOOGLE_SHEETS_ID;
  ...
  if (!sheets.length) {
    try {
      ...
      if (rows) {
        ...
        if (w) {
          const c = await fbGet(env, base + "/schedule");
          if (equal(c, w)) return json({ updated: 0, notes: ["gviz fallback: без изменений"] });
          await fbPut(env, base + "/schedule", w);
  ...
  const current = await fbGet(env, base + "/schedule");
  if (equal(current, days)) {
    console.log("Расписание не изменилось (updated=0)");
    return json({ updated: 0 });
  }
  await fbPut(env, base + "/schedule", days);
...
const DEFAULT_NIKA_URL = "https://raspisanie.nikasoft.ru/86111512.html";
...
async function nikaScheduleId(env) {
  console.log("Скачиваю HTML raspisanie.nikasoft.ru…");
  const html = await httpText((env && env.NIKA_URL) || DEFAULT_NIKA_URL);
  const m = /initial_schedule_id\s*=\s*['"]([^'"]+)['"]/.exec(html);
  if (!m) throw new Error("нет переменной initial_schedule_id");
  return m[1];
}

async function nikaLoad(env) {
  const id = await nikaScheduleId(env);
...
async function firebaseClassBySid(env, cid, sid, name) {
  const target = normKey(name);
  const cls = await fbGet(env, "cities/" + cid + "/schools/" + sid + "/classes");
  if (!cls) return null;
...
async function handleParseNikasoft(body, env) {
  const errors = [];
  const cid = String(body.cid || ""), sid = String(body.sid || ""), clid = String(body.clid || "");
  if (!cid || !sid || !clid) return json({ updated: 0, errors: ["нет cid/sid/clid"] }, 400);
  ...
  if (allClasses) {
    ...
    const targetClid = clid || await firebaseClassBySid(env, cid, sid, className);
...
// ============================================================================
// Диспетчер (Cloudflare Worker: export default { fetch })
// ============================================================================
...
export default {
  async fetch(request, env) {
    ...
    try {
      if (url.pathname === "/moderate") return await handleModerate(body, env);
      if (url.pathname === "/parse/google") return await handleParseGoogle(body, env);
      return await handleParseNikasoft(body, env);
    } catch (e) {
